...
{
	echo "正在检查依赖...."
	verions_arr=$(mvn versions:display-property-updates -Prelease |fgrep venus|fgrep [INFO]|fgrep -v Building|awk '{ print $4}')
	...
	do
		...
		then
			...
			echo "####################################BEGIN##################################"
			mvn versions:display-property-updates -Prelease |fgrep venus|fgrep SNAPSHOT | grep -v Building
			echo "####################################END####################################"
			exit 1
		fi
	done
...
{
    WAR_NAME=${PROJECT_NAME}-${TAR_VERSION}.war
    set -e
    echo "--------maven--打包成功，准备tar.gz,并放入nginx文件服务器目录-------"
    cd ${BASE_PATH}/${WORK_BRANCH}/target
    mkdir -p ${NGINX_FILE_PATH}/${HISTORY_PATH}

    mv -f ${WAR_NAME} ${NGINX_FILE_PATH}
...
{
    cd ${WORK_BRANCH}
    PROJECT_VERSION=`mvn clean -U|fgrep "Building ${PROJECT_NAME}"|awk '{print $NF}'`
...
function release_mvn_pkg()
{
    ...
    mvn install -U -e -Dmaven.test.skip=true || error_rollback

    echo "====5====:此版本对其他项目依赖和最新版本对比==============">>${TEMP}
    mvn versions:display-property-updates -Prelease|grep venus | grep "\\$">>${TEMP}
    echo "==========================================================">>${TEMP}
    #分支提交到tags
...
{       
	...
	mvn clean install -Dmaven.test.skip=true -X -U -e
	echo "====2====:SNAPSHOT版本install成功:${PROJECT_VERSION}-------">>${TEMP}
}

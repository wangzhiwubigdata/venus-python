...
#这个脚本只试用venus-api-servlet，因为它有-Prelease变量
...
function release_mvn_pkg()
{
    ...
    mvn clean install -U -e -Prelease -Dmaven.test.skip=true || error_rollback
...
{       
	...
	#mvn deploy -e -U -Dmaven.test.skip=true
	mvn clean install -e -U -Dmaven.test.skip=true
	echo "====2====:SNAPSHOT版本jar包上传私服成功:${PROJECT_VERSION}-------">>${TEMP}
}

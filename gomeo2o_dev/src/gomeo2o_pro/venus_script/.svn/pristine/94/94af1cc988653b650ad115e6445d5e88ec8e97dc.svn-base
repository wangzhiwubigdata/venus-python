...
{
	echo "正在检查依赖...."
	#verions_arr=$(mvn versions:display-property-updates |fgrep venus|awk '{ print $4}')
...
function get_project_version()
{
    ...
    echo "====1====:原项目版本号为:${PROJECT_VERSION}--------">>${TEMP}
}
#错误回滚
...
function release_mvn_pkg()
{
    ...
    echo "====2====:发布版本号为:${RELESE_VERSION}-------">>${TEMP}
    ...
    echo "====3====:开发版本被更新为:${NEW_SNAPSHOT}-------">>${TEMP}
    ...
    mvn deploy -e -Dmaven.test.skip=true || error_rollback
            echo "----deploy到私服成功--------"
    echo "====4====:发版版本jar包上传私服成功:${RELESE_VERSION}-------">>${TEMP}
    ...
    svn ci ${BASE_PATH}/tags/${RELESE_VERSION} -m "cp release to tags" || error_rollback

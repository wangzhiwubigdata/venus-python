...
class UserTester (TesterBase):
    def u_expert_types(self):
        o = self.user_get_v1('user/query_expert_types.json?')
        return self.ret(o, lambda: o['data']['result'][1]['labelId'] > 0)

    def u_experts_by_type(self):
#         o = self.user_get_v1('user/query_expert_by_type.json?labelId=1&')
#         return self.ret(o, lambda: o['data']['result'][0]['userId'] > 0)
        pass

    def othermember_info(self):
        o = self.comm_get_v1('user/get_othermember_info.json?memberId={uid}&', uid=self.cfg['userId'])
        return self.ret(o, lambda: o['data']['imId'] == self.cfg['imId'])

    def u_sharerecord_list(self):
#         o = self.user_get_v1('user/get_sharerecord_list.json?memberId={uid}&', uid=self.cfg['userId'])
#         return self.ret(o, lambda: o['data']['result'][0]['productId'] > 0)
        pass

    def u_personal_info(self):
        o = self.user_get_v1('user/get_personal_info.json?')
        return self.ret(o, lambda: o['data']['userId'] > 0)

    def u_my_acount(self):
        o = self.user_get_v1('account/my_acount.json?')
        return self.ret(o, lambda: True)

    def u_shop_collection(self):
        o = self.user_get_v1('user/shop_collection.json?')
        return self.ret(o, lambda: True)

    def u_product_collection(self):
        o = self.user_get_v1('user/product_collection.json?')
        return self.ret(o, lambda: True)

    def _query_vshop_info_by_user_id(self):
        # o = self.user_get_v1('vshop/query_vshop_info_by_user_id.json?')
        # p(o)
        pass

    def _shop_detail(self):
        # o = self.user_get_v1('shop/shop_detail.json?shopId=1&')
        # p(o)
        pass

    def feedback_type_list(self):
        o = self.comm_get_v1('user/get_feedback_type_list.json?')
        return self.ret(o, lambda: True)

    def u_feedback_list_1(self):
        o = self.user_get_v1('user/feedback_list_1.json?')
        return self.ret(o, lambda: True)

    def u_topic_collect_list(self):
        o = self.user_get_v1('user/get_topic_collect_list.json?')
        return self.ret(o, lambda: True)

    def u_address_detail(self):
        o = self.user_get_v1('user/address_detail.json?addressId={addrId}&', addrId=self.cfg['addrId'])
        return self.ret(o, lambda: o['data']['userId'] > 0)

    def u_address_list(self):
        o = self.user_get_v1('user/address_list.json?')
        return self.ret(o, lambda: True)

    def u_referee_list(self):
        o = self.user_get_v1('user/get_referee_list.json?')
        return self.ret(o, lambda: True)

    def get_personal_info_by_mobile(self):
#         o = self.comm_get_v1('user/get_personal_info_by_mobile.json?mobile=18911261622&')
#         return self.ret(o, lambda: True)
        pass

    def shop_detail(self):
        o = self.comm_get_v1('shop/shop_detail.json?shopId={shopid}&', shopid=self.cfg['shopId'])
        return self.ret(o, lambda: True)

    def item_list(self):
        o = self.comm_get_v1('vshop/item_list.json?shopId={shopid}&type=1&', shopid=self.cfg['shopId'])
        return self.ret(o, lambda: True)

    def u_item_manage_list(self):
        o = self.user_get_v1('vshop/item_manage_list.json?shopId={shopid}&type=1&', shopid=self.cfg['shopId'])
        return self.ret(o, lambda: True)

    def u_vshop_category_list(self):
        o = self.user_get_v1('vshop/query_vshop_category_list.json?vshopId={vshopid}&', vshopid=self.cfg['vshopId'])
        return self.ret(o, lambda: True)

    def u_distribution_item_list(self):
        o = self.user_get_v1('vshop/query_distribution_item_list.json?shopId={shopid}&', shopid=self.cfg['shopId'])
        return self.ret(o, lambda: True)

    def u_vshop_product_list_by_category(self):
        o = self.user_get_v1('vshop/query_vshop_product_list_by_category.json?vshopId={vshopid}&categoryId=1&categoryName=1&roleId=1&', vshopid=self.cfg['vshopId'])
        return self.ret(o, lambda: True)

    def get_freight_template_list(self):
        o = self.comm_get_v1('vshop/get_freight_template_list.json?')
        return self.ret(o, lambda: True)

    # v2
    def user_info(self):
        c, o = self.comm_get_v2('user/user?id={uid}&integrity=full&', uid=self.cfg['userId'])
        return self.ret_v2(c, o, lambda: o['data']['id'] == int(self.cfg['userId']))

    def my_interest_categories(self):
        c, o = self.user_get_v2('user/myInterestCategories?')
        return self.ret_v2(c, o, lambda: type(o['data']['interestCategories']) == type([]))

    def personal_info(self):
        c, o = self.user_get_v2('user/personalInfo?')
...
#     ('u_product_collection',    u'*138_产品收藏列表', leaders + yangyanyan),
    ('u_my_acount',             u'166_我的账户首页', leaders),
